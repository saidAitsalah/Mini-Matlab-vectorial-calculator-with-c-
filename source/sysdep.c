...
void sys_out_mode(int mode)
{
    ...
    case CC_WARN:
    case CC_ERROR:
        uart_puts(USART0, "\x1b[31m");  // Réinitialise la couleur à la couleur par défaut.
        break;
    case CC_EDIT:
        uart_puts(USART0, "\x1b[32m"); //  la couleur du texte en vert.
        uart_puts(USART0, "> ");
        uart_puts(USART0, "\x1b[0m");  //  la couleur à la couleur par défaut après l'affichage.
        break;
    case CC_FEDIT:
        uart_puts(USART0, "\x1b[31m"); //  la couleur du texte en rouge.
        uart_puts(USART0, "$ ");
        uart_puts(USART0, "\x1b[0m");  //  la couleur à la couleur par défaut après l'affichage.
        break;
    default:
...
void sys_clear(void)
{
    //  code VT100 pour effacer l'écran.
    const char clear_screen[] = "\x1b[2J";
    uart_puts(USART0, clear_screen);
}

void move_cl_cb(void)
{
    //  code VT100 pour déplacer le curseur d'une position vers la gauche.
    const char move_left[] = "\x1b[1D";
    uart_puts(USART0, move_left);
}

void move_cr_cb(void)
{
    // code VT100 pour déplacer le curseur d'une position vers la droite.
    const char move_right[] = "\x1b[1C";
    uart_puts(USART0, move_right);
}

void cursor_on_cb(void)
{
    // code VT100 pour activer le curseur.
    const char cursor_on[] = "\x1b[?25h";
    uart_puts(USART0, cursor_on);
}

void cursor_off_cb(void)
{
    // code VT100 pour désactiver le curseur.
    const char cursor_off[] = "\x1b[?25l";
    uart_puts(USART0, cursor_off);
}

void clear_eol(void)
{
    // code VT100 pour effacer jusqu'à la fin de la ligne.
    const char clear_line[] = "\x1b[K";
    uart_puts(USART0, clear_line);
}
...
real sys_clock(void)
{
    ...
    config.prescale = 0;  // Ajustement le prédiviseur
    ctimer_match_config_t matchConfig;
    matchConfig.enableCounterReset = true;
    matchConfig.enableCounterStop = false;
    matchConfig.matchValue = 0;  // Compter les secondes
    CTIMER_Init(CTIMER0, &config);
    CTIMER_SetupMatch(CTIMER0, kCTIMER_Match_0, &matchConfig);
    CTIMER_StartTimer(CTIMER0);

    return (real)(CTIMER_GetTimerCountValue(CTIMER0));
}

#define UART_TIMEOUT_MS 1000 // par exemple, 1000 ms (1 seconde)

char uart_getc_timeout(uint8_t uart, uint32_t timeout_ms)
{
    uint32_t startTime = sys_clock(); // Obtenir le temps de début en secondes

    while ((sys_clock() - startTime) < (timeout_ms / 1000))
    {
        // Lire un caractère du port série
        char c = uart_getc(uart);
        if (c != 0)
        {
            return c; // Retourner le caractère lu
        }
    }

    return 0; // Aucun caractère lu avant le timeout
}
...
void sys_wait(real time, scan_t *scan)
{
    ...
    matchConfig.enableCounterReset = true;
    matchConfig.enableCounterStop = false;
    uint32_t matchValue = (uint32_t)(time * 1000); // Convertir le temps en millisecondes
    CTIMER_SetupMatch(CTIMER0, kCTIMER_Match_1, &matchConfig); // Configurer la temporisation

    *scan = 0; // Initialiser la valeur de scan
    ...
    while (!(*scan))
    {
        ...
        if ((sys_clock() - startTime) >= (UART_TIMEOUT_MS / 1000))
        {
            // Le timeout est atteint, sortir de la boucle
            break;
        }

        // Lire un caractère du port série avec une temporisation
        char c = uart_getc_timeout(USART0, UART_TIMEOUT_MS);

        if (c == 0x1b)
        {
            *scan = escape; // La touche ESC a été pressée
        }
        else if (c == '\r' || c == '\n')
        {
            *scan = enter; // La touche Enter a été pressée
        }
    }
}
...
int _open(const char *name, int flags, int mode) {
	FRESULT res;
	    ...
	    res = f_open(&files[fd], name, FA_READ | FA_WRITE | FA_OPEN_ALWAYS);
	    if (res != FR_OK) {
	        return -1;
	    }
...
int _read(int fd, char *ptr, int len) {
	...
	fd -= 3;
	    ...
	    if (files[fd].obj.fs) {
	        UINT bytesRead;
	        FRESULT res = f_read(&files[fd], ptr, len, &bytesRead);
	        if (res == FR_OK) {
	            return bytesRead;
	        } else {
	            return -1;
...
int _write(int fd, char *ptr, int len) {
	...
	fd -= 3;
	    ...
	    if (files[fd].obj.fs) {
	        UINT bytesWritten;
	        FRESULT res = f_write(&files[fd], ptr, len, &bytesWritten);
	        if (res == FR_OK) {
	            return bytesWritten;
	        } else {
	            return -1;
